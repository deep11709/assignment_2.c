...
int base(long long int n)//function for finding the base
{
  ...
  while (n>0)//loop will work to check which no. is greatest in all
  {
    ...
    {
      max = r;//max no. will store greatest digit of the generated no.
    }
...
int main()
{
  ...
  int greatest; // variable to store the result of function
  ...
  greatest = base(arr[i]);//function called to check the base

  printf("\n The Highest Base of %lld is %d \n", arr[i], greatest);//final result
  }
